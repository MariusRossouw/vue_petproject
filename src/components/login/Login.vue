...
<script>
  export default {
    ...
    //   let mobile = document.cookie.match(/^(\+?\d{1,4}[\s-])?(?!0+\s+,?$)\d{10}\s*,?$/)
      let password = document.cookie.match('(^|;)\\s*' + 'password' + '\\s*=\\s*([^;]+)')
      this.$refs.txtEmail.value = email ? email.pop() : ''
    //   this.$refs.txtMobile.value = mobile ? mobile.pop() : ''
    ...
    //   },
      ...
      submitLogin: function () {
        ...
        //   document.cookie = 'mobile=' + mobile + ';expires=' + d.toUTCString() + ';path=/'
